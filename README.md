<div align="center">
  <a href="#">
    <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=200&section=header&text=Sandunika%20Herath!&fontSize=80&fontColor=ffffff&color=0:8b5cf6,50:a855f7,100:c084fc&animation=fadeIn" />
  </a>
</div>

<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=A855F7&background=00000000&center=true&vCenter=true&width=600&height=60&lines=Full+Stack+Developer+%7C+Web+Developer;Building+Digital+Health+Solutions;Passionate+About+Clean+Code;Laravel+%26+Java+Enthusiast;Always+Learning+%26+Growing" alt="Typing Animation" />
</div>

<h3 align="center" style="color:#A855F7;">🚀 Crafting Digital Solutions with Purpose</h3>

<div align="center" style="margin: 20px 0;">
  <img src="https://img.shields.io/github/followers/sanduherath?label=Followers&style=for-the-badge&color=C084FC&labelColor=8B5CF6" alt="Followers" />
  <img src="https://img.shields.io/badge/Status-Open%20to%20Opportunities-A855F7?style=for-the-badge&labelColor=8B5CF6" alt="Status" />
  <img src="https://img.shields.io/badge/Location-Kandy%2C%20Sri%20Lanka-C084FC?style=for-the-badge&labelColor=8B5CF6" alt="Location" />
</div>

---

```javascript
const fullStackDeveloper = {
  name: "Sandunika Herath",
  location: "Kandy, Danthure, Sri Lanka",
  email: "sanduherath0905@gmail.com",
  phone: "+94752735746",
  currentEducation: "HND in Information Technology (Expected 2025)",
  specialization: ["Web Development", "Healthcare Solutions", "Full Stack Development"],
  languages: ["Java", "Python", "PHP", "JavaScript", "SQL"],
  expertise: ["Laravel Framework", "MVC Architecture", "Database Design", "UI/UX Development"],
  currentFocus: ["AI/ML Engineering", "Digital Health Platforms", "Responsive Web Design"],
  
  techStack: {
    frontend: ["HTML5", "CSS3", "JavaScript", "Bootstrap", "Responsive Design"],
    backend: ["Laravel", "PHP", "Java", "Python"],
    database: ["MySQL", "Database Architecture"],
    tools: ["Git", "GitHub", "MVC Pattern", "Object-Oriented Programming"]
  },
  
  featuredProjects: [
    "🍼 Baby Tracking System - Digital health monitoring platform",
    "📚 Library Management System - Java MVC architecture",
    "🏦 Banking Management System - Secure web application",
    "🎬 Cinema Majestic - Front-end development"
  ],
  
  passions: [
    "Healthcare Technology",
    "User-Friendly Design",
    "Problem Solving",
    "Continuous Learning",
    "Innovation in Web Development"
  ],
  
  motto: "Creating user-friendly, innovative digital solutions that make a difference."
};

console.log(`
╔══════════════════════════════════════════════════════════════╗
║  💡 Welcome to my development journey!                      ║
║  🌟 Transforming ideas into meaningful web applications     ║
║  🔧 Specializing in healthcare and management systems       ║
║  🚀 Building tomorrow's digital solutions today             ║
╚══════════════════════════════════════════════════════════════╝
`);
```

---

<h3 align="center">🎯 Current Journey & Learning Path</h3>

<div align="center">

| 🔷 **Currently Building** | 💜 **Learning** | 🔹 **Next Goals** |
|:------------------------:|:---------------:|:------------------:|
| Health Monitoring Systems | AI/ML Engineering | Cloud Technologies |
| Laravel Applications | Advanced Python | Mobile Development |
| Responsive Web Interfaces | Database Optimization | DevOps Practices |

</div>

---

<h3 align="center">🏆 Featured Projects</h3>

<div align="center">
<table>
<tr>
<td width="50%">

### 🍼 Baby Tracking System
**Laravel | MySQL | JavaScript | Bootstrap**
- Digital health monitoring platform
- Automated vaccination scheduling
- Role-based dashboards (MOH, midwives, parents)
- Real-time notification alerts
- [View on GitHub](https://github.com/sanduherath/baby-tracker)

### 🏦 Banking Management System
**PHP | MySQL | JavaScript | Bootstrap**
- Secure web-based banking platform
- Role-based access control
- Real-time transaction tracking
- Responsive design across devices

</td>
<td width="50%">

### 📚 Library Management System
**Java | MVC Architecture**
- Complete library operations management
- User authentication & authorization
- Object-Oriented Programming principles
- Structured code design

### 🎬 Cinema Majestic
**HTML | CSS | JavaScript | Bootstrap**
- Cinema management system frontend
- Responsive layouts & intuitive navigation
- Interactive user elements
- [View on GitHub](https://github.com/sanduherath/TMS-Majestic-Cinema-Loft-)

</td>
</tr>
</table>
</div>

---

<h3 align="center">📊 GitHub Insights</h3>

<div align="center">
<table align="center">
<tr border="none">
<td width="50%" align="center">
  
  <img align="center" src="https://github-readme-stats.vercel.app/api?username=sanduherath&theme=tokyonight&show_icons=true&count_private=true&border_color=A855F7&title_color=8B5CF6&icon_color=C084FC&text_color=E5E7EB&bg_color=1F2937"/>
  
  <br/><br/>
  
  <img align="center" src="https://streak-stats.demolab.com/?user=sanduherath&theme=tokyonight&border=A855F7&ring=8B5CF6&fire=C084FC&currStreakLabel=E5E7EB&sideLabels=E5E7EB&dates=9CA3AF"/>
  
</td>
<td width="50%" align="center">

  <img align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=sanduherath&theme=tokyonight&layout=compact&border_color=A855F7&title_color=8B5CF6&text_color=E5E7EB&bg_color=1F2937"/>
  
  <br/><br/>
  
  <img align="center" src="https://github-readme-activity-graph.vercel.app/graph?username=sanduherath&theme=tokyo-night&color=A855F7&line=C084FC&point=8B5CF6&area=true&hide_border=true"/>
  
</td>
</tr>
</table>
</div>

---

<h3 align="center">🛠️ Technology Stack</h3>

<div align="center">
  
**Programming Languages**
<br/>
<img src="https://skillicons.dev/icons?i=java,python,php,js,html,css&theme=light" />

**Frameworks & Libraries**
<br/>
<img src="https://skillicons.dev/icons?i=laravel,bootstrap&theme=light" />

**Database & Tools**
<br/>
<img src="https://skillicons.dev/icons?i=mysql,git,github,vscode&theme=light" />

**Currently Learning**
<br/>
<img src="https://skillicons.dev/icons?i=tensorflow,pytorch,aws,docker&theme=light" />

</div>

---

<h3 align="center">🎓 Education & Certifications</h3>

<div align="center">

| 🎓 **Education** | 📜 **Certifications** |
|:---------------:|:--------------------:|
| **HND in Information Technology** <br/> Advanced Technological Institute - Kandy <br/> *(Expected 2025)* | **AI/ML Engineer Stage 1** <br/> SLIIT |
| **G.C.E Advanced Level** <br/> Seethadevi Girls' College <br/> *(Physical Science Stream)* | **Cloud Foundations** <br/> NextGen Campus |

</div>

---

<h3 align="center">💼 Professional Skills</h3>

<div align="center">
<table>
<tr>
<td align="center" width="33%">

**🔧 Technical Skills**
- Java Programming
- Web Development (HTML, CSS, JS)
- Laravel Framework
- Database Design
- MVC Architecture

</td>
<td align="center" width="33%">

**🤝 Soft Skills**
- Quick Learner
- Team Collaboration
- Time Management
- Problem Solving
- Leadership

</td>
<td align="center" width="33%">

**🎯 Specializations**
- Healthcare Solutions
- Management Systems
- Responsive Design
- User Experience
- System Architecture

</td>
</tr>
</table>
</div>

---

<h3 align="center">🌐 Let's Connect & Collaborate</h3>

<div align="center">
  <a href="https://www.linkedin.com/in/sandu-herath" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/>
  </a>
  <a href="mailto:sanduherath0905@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Email-A855F7?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/>
  </a>
  <a href="https://github.com/sanduherath" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
  </a>
  <a href="tel:+94752735746" target="_blank">
    <img src="https://img.shields.io/badge/Phone-C084FC?style=for-the-badge&logo=whatsapp&logoColor=white" alt="Phone"/>
  </a>
</div>

---

<div align="center">
  <h3>📈 Contribution Activity</h3>
  <img src="https://github-readme-stats.vercel.app/api/wakatime?username=sanduherath&theme=tokyonight&border_color=A855F7&title_color=8B5CF6&text_color=E5E7EB&bg_color=1F2937" alt="Wakatime Stats" />
</div>

---

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=0:8b5cf6,50:a855f7,100:c084fc" />
</div>

<h4 align="center">💜 Thanks for visiting! Let's build innovative solutions together 🚀</h4>

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=sanduherath&label=Profile%20Views&color=A855F7&style=for-the-badge" alt="Profile Views" />
</div>
